...
def extraer_columnas_csv(ruta_csv):
    ...
    with open(ruta_csv, mode='r', newline='', encoding='ISO-8859-1') as archivo_csv:
        ...
        for fila in lector_csv:
            ...
            sub_categoria = "a"
            proveedor = "air"
            precio_dolares = fila["lista5"]
            precio_dolares_iva = float("{:.2f}".format(
                float(fila["lista5"]) * (float(fila["IVA"]) / 100 + 1)))
            iva = float(fila["IVA"])
            precio_pesos = float(fila["lista5"]) * float(dolar_venta)
            precio_pesos_iva = float("{:.2f}".format(
                float(fila["lista5"]) * float(dolar_venta) * (float(fila["IVA"]) / 100 + 1)))
            ...
            if (categoria != 0 and len(codigo_fabricante) > 2):
                parametros = (nombre, stock, garantia_meses, detalle, largo, alto, ancho, peso, codigo_fabricante, marca, categoria,
                              sub_categoria, proveedor, precio_dolares, precio_dolares_iva, iva, precio_pesos, precio_pesos_iva, url_imagen, deposito)
                try:
                    ...
                    print(
                        f"Error al ejecutar el procedimiento almacenado: {error}")

    # Llama a la función para procesar las filas seleccionadas
# Ejemplo de uso:
ruta_csv = './arituc.csv'  # Cambia esto por pya ruta a tu archivo CSV
# Cambia esto por las columnas que quieras extraer
columnas_interes = ['nombre', 'edad', 'correo']
